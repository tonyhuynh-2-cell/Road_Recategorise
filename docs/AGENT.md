...
- **The B-double 80% coverage threshold** — this IS flagged as possibly wrong (see `DECISIONS.md` D-06 and `STATUS.md` known bugs), but the fix should come from evidence-based investigation (use the Criteria Overrides slider to explore first), not an arbitrary change. Don't silently lower it in the pipeline without documenting the before/after impact and ideally confirming with the client.
