# -*- coding: utf-8 -*-
"""
Rebuild the URBAN State facility / employment optional criterion (S-11).
...
SCOPE: URBAN roads only. Rural S-08 is owned by
rebuild_state_facility_optional.py (NSW Road Segment network components) —
this script never touches rural values. The computed value is stored in
stateOpt.dest* for every URBAN road (merged beside existing stateOpt keys,
never replacing them; dest_method 'sal_evidence_components') so urban
Regional roads use it when cross-tested as State. Urban State roads also
...
def evaluate_state_dest(lines, evidence_row, area):
    ...
    return {
        ...
        "dest_method": "sal_evidence_components",
...
derived = {rn: evaluate_state_dest(road_lines.get(rn, []), evidence.get(rn), c["area"])
           for rn, c in criteria.items() if c["area"] == "urban"}

changed = {}
info_changed = 0
for rn, c in criteria.items():
    if rn not in derived:
        continue
    info = derived[rn]
...
log(f"urban roads with new State facility metadata: {info_changed}")
log(f"urban State roads with S-11 score changes: {len(changed)}")
...
for row in exports["state"]:
    ...
    if not c or c["cls"] != "State":
        continue
    if c["area"] != "urban":
        continue          # rural S-08 rows belong to rebuild_state_facility_optional.py
    code = "S-11"
